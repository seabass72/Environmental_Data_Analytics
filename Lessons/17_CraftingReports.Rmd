...
Data <- c("EPA Air PM25 & O3", "ECOTOX Neonicotinoids", "NTL LTER Lake Physical and Chemical Data", "USGS Flow Data")
Comments<-c("ECOTOX of harmful chemicals in waterways", "EPA air quality data", "long term ecological research concerning physical and chemical characteristics", "usgs flow and discharge data on the eno river")
df <- data.frame(Data, Comments)
library(knitr)
kable(df )

```

- code output and text in one document 
- knit to useful formats and PDF and HTML 
...
```{r echo = FALSE}
NTL.LTER <- read.csv("./Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv")

head(NTL.LTER)

# set the date to date format

NTL.LTER$sampledate<- as.Date(NTL.LTER$sampledate, "%m/%d/%y")
```
